...
try {
    ...
    //获取我的动态列表
    $newsList = array();
    $ret = $db->query("SELECT news_id,news_time FROM news WHERE news_user_id = '$myUserId' ORDER BY news_time DESC ");
    while ($row = $ret->fetchArray()) {
        $news = array();
        $newsId = $row['news_id'];
        $news['newsId'] = $newsId;
        ...
        <main>
            ...
            <section>
                <div id="friendsNewsContainer" class="uk-flex uk-flex-center">
                    <ul class="uk-list uk-list-divider friend-news-list">
                        <?php
                        if (empty($newsList)) {
                            echo '<li><p class="uk-text-center">暂无动态哦</p></li>';
                        } else {
            ...
            <section class="uk-padding friends-list-container">
                ...
                <div class="uk-grid uk-margin-remove uk-child-width-1-3 friends-list" uk-grid>
                    <?php
                    if (empty($friendList)){
                        echo '<p class="uk-text-center">暂无关注哦</p>';
                    }
                    else {
